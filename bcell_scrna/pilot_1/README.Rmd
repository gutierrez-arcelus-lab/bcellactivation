...
plot_markers <- function(cluster_df) {
  ...
  cluster_cell <- 
    tibble(cluster_cell = Idents(bcells_singlet),
...
markers_timep_df <- bcells_markers_24 %>%
  mutate(cluster_top = ifelse(avg_log2FC > 0, "BCR 24hr", "TLR7 24hr")) %>%
  left_join(select(features_df, 1:2), by = c("gene" = "gene_id")) %>%
  pivot_longer(`BCR 24hr`:`TLR7 24hr`, names_to = "cluster", values_to = "pct")
...
timep_gene_avg_expr <- cell_gene_expr_timep %>%
  ...
  filter(cluster_cell %in% c("BCR 24hr", "TLR7 24hr")) %>%
