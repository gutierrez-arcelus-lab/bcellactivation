...
```{r, echo=FALSE}
stim_colors <- 
    c("grey75", "grey60", "gold1", "gold3", "mediumpurple1", "mediumpurple4")
...
write_rds(bcells_singlet, "./scDRS/data/bcells_singlet_seurat.rds")
```
...
#magma_genes_annot <- "./scDRS/data/gencode_genes/genes.v19.hg19.loc" %>%
magma_genes_annot <- "./scDRS/data/gencode_genes/genes.sle.loc" %>%
  read_tsv(col_names = FALSE) %>%
  select(gene_id = X1, gene_name = X6, chr = X2, pos = X3) %>%
  extract(gene_name, c("ix", "gene_name"), "(\\d+)_(.+)")

#magma <- read_table("./scDRS/results/Bentham.genes.out") %>%
magma <- read_table("./scDRS/results/SleRiskGenes.genes.out") %>%
  select(gene_id = GENE, z = ZSTAT, p = P) %>%
  left_join(magma_genes_annot, by = "gene_id") %>%
  mutate(abs_z = abs(z),
         logp = -log10(p)) %>%
  arrange(desc(abs_z))
...
```{r, echo=FALSE, fig.height=8}
#scdrs_scores <- read_tsv("./scDRS/results/SLE.score.gz") %>%
scdrs_scores <- read_tsv("./scDRS/results/SleRiskGenes/SLE.score.gz") %>%
  rename("barcode" = "X1")
...
scdrs_1 <- ggplot(umap_scdrs_df, aes(UMAP_1, UMAP_2, color = norm_score)) +
    geom_point(size = 1) +
...
scdrs_2 <- ggplot(filter(umap_scdrs_df, pval >= 0.05),
                  aes(UMAP_1, UMAP_2)) +
    geom_point(size = 1, color = "white") +
    geom_point(data = filter(umap_scdrs_df, pval < 0.05),
    ...
    theme(panel.grid = element_blank(),
          panel.background = element_rect(fill = "grey")) +
    guides(color = guide_legend(override.aes = list(size = 2))) +
    annotate("text", x = 5, y = 5, label = "*only coloring cells with p < 0.05")


plot_grid(scdrs_1, scdrs_2, ncol = 1)
```
...
markers_timep_df %>%
  filter(time == "24hr") %>%
  rename("stim" = "cluster") %>%
  mutate(cluster = paste0(stim, time),
         cluster = sub("hr$", "", cluster)) %>%
  left_join(select(timep_gene_avg_expr, cluster, gene_name, scale_expr)) %>%
  arrange(desc(avg_log2FC)) %>%
  mutate(gene_name = fct_inorder(gene_name),
         tier = ntile(1:nrow(.), 2)) %>%
  ggplot(aes(cluster, gene_name)) +
  geom_point(aes(size = pct, color = scale_expr)) +
