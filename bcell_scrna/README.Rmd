...
stim_class_df <- tibble(barcode = colnames(bcells_singlet@assays$HTO@data),
                        stim = bcells_singlet@meta.data$HTO_maxID)

hto_df <- bcells_singlet@assays$HTO@data %>%
  as.data.frame() %>%
  rownames_to_column("hto") %>%
  as_tibble() %>%
  pivot_longer(-hto, names_to = "barcode") %>%
...
hto_top <- hto_df %>%
  ...
  select(barcode, top_hto = hto)

hto_df %>%
  left_join(hto_top) %>%
  mutate(top_hto = factor(top_hto, levels = names(stim_colors)),
         stim = factor(stim, levels = names(stim_colors))) %>%
  ggplot(aes(reorder_within(barcode, by = value, within = stim), value)) +
  geom_col(aes(fill = hto), position = "fill", width = 1.05) +
  scale_fill_manual(values = stim_colors) +
...
plot_grid(plotlist = clustering_plot_list, ncol = 1)
```
...
top_marker_summary <- top_marker_expr %>%
  ...
  arrange(cluster_top, desc(avg_log2FC)) %>%
  mutate(gene_name = fct_inorder(gene_name))
...
ggplot(top_marker_summary, aes(cluster, gene_name)) +
  ...
  geom_text(data = distinct(top_marker_summary, cluster = cluster_top, gene_name),
...
```
```{r, echo=FALSE}
markers_time72_df <- bcells_markers_72 %>%
  mutate(cluster_top = ifelse(avg_log2FC > 0, "IgG72", "RSQ72")) %>%
  left_join(select(features_df, 1:2), by = c("gene" = "gene_id")) %>%
  pivot_longer(IgG72:RSQ72, names_to = "cluster", values_to = "pct")

cell_gene_expr_time72 <- bcells_singlet@assays$RNA@data %>%
  .[unique(markers_time72_df$gene), ] %>%
  as.data.frame() %>%
  rownames_to_column("gene_id") %>%
  as_tibble() %>%
  pivot_longer(-gene_id, names_to = "barcode", values_to = "logexpr")

time72_gene_avg_expr <- cell_gene_expr_time72 %>%
  left_join(cluster_cell) %>%
  filter(cluster_cell %in% c("IgG72", "RSQ72")) %>%
  group_by(cluster = cluster_cell, gene_id) %>%
  summarise(scale_expr = mean(logexpr)) %>%
  ungroup() %>%
  left_join(select(features_df, gene_id, gene_name)) %>%
  left_join(markers_time72_df, by = c("gene_id" = "gene", "gene_name", "cluster")) %>%
  arrange(avg_log2FC) %>%
  mutate(gene_name = fct_inorder(gene_name))

sle_candidates_label_72 <- time72_gene_avg_expr %>%
  inner_join(sle_candidates)  %>%
  distinct(cluster = cluster_top, gene_name) %>%
  mutate(i = "*")
```
...
de_72_1 <- ggplot(time72_gene_avg_expr %>% filter(cluster_top == "IgG72"), 
       aes(cluster, gene_name)) +
  geom_point(aes(size = pct, fill = scale_expr), 
             color = "black", shape = 21) +
  scale_size(range = c(0.05, 2.5), labels = scales::percent) +
  scale_fill_viridis_c(option = "magma") +
  scale_x_discrete(position = "top") +
  geom_text(data = sle_candidates_label_72 %>% filter(cluster == "IgG72"), 
            aes(label = i), 
            size = 3, nudge_x = -.1, nudge_y = -.25) +
  theme_bw() +
  theme(axis.line = element_blank(),
  ...
  labs(x = NULL, y = NULL, 
       fill = "Scaled\nExpression", 
       size = "% of cells",
       title = "Markers IgG 72hr") +
  guides(fill = guide_colorbar(barwidth = .5))

de_72_2 <- ggplot(time72_gene_avg_expr %>% filter(cluster_top == "RSQ72"), 
       aes(cluster, gene_name)) +
  geom_point(aes(size = pct, fill = scale_expr), 
             color = "black", shape = 21) +
  scale_size(range = c(0.05, 2.5), labels = scales::percent) +
  scale_fill_viridis_c(option = "magma") +
  scale_x_discrete(position = "top") +
  geom_text(data = sle_candidates_label_72 %>% filter(cluster == "RSQ72"), 
            aes(label = i), 
            size = 3, nudge_x = -.1, nudge_y = -.25) +
  theme_bw() +
  theme(axis.line = element_blank(),
  ...
  labs(x = NULL, y = NULL, 
       fill = "Scaled\nExpression", 
       size = "% of cells",
       title = "Markers RSQ 72hr") +
   guides(fill = guide_colorbar(barheight = .5),
          size = guide_legend(keywidth = .6))

de_72_grid <- 
  plot_grid(de_72_2 + theme(legend.position = "none"),
          get_legend(de_72_2), 
          ncol = 1, rel_heights = c(.66, .34))

plot_grid(de_72_1 + theme(legend.position = "none"), 
          de_72_grid, nrow = 1)
```
...
```{r microgrant, include=FALSE, echo=FALSE, eval=FALSE, fig.height=6, fig.width=9}
mc_plot <- umap_df %>%
  ...
  ggplot(aes(UMAP_1, UMAP_2, color = stim)) +
    geom_point(size = .5) +
    scale_color_manual(values = setNames(stim_colors, NULL)) +
    theme_bw() +
    theme(panel.grid = element_blank(),
          panel.border = element_blank(),
          axis.line = element_blank(),
          ...
          plot.margin = margin(0, .5, .5, 0, unit = "cm")) +
    guides(color = guide_legend(override.aes = list(size = 2, alpha = 1))) +
  labs(color = NULL)

mc_genes <- c("BLK", "SOCS1", "NCF2", "IRAK1")

mc_prots <- c("IgD", "CD27", "CD11c")
...
mc_adt_quants <- bcells_singlet@assays$ADT@data[mc_prots, ] %>%
    as.data.frame() %>%
    rownames_to_column("ab") %>%
    as_tibble() %>%
    pivot_longer(-ab, names_to = "barcode", values_to = "ab_level") %>%
    mutate(ab = factor(ab, levels = mc_prots))

mc_prots_plot_list <- umap_df %>%
    select(barcode, UMAP_1, UMAP_2) %>%
    left_join(mc_adt_quants, by = "barcode") %>%
    split(.$ab) %>%
    map(~ggplot(data = ., aes(UMAP_1, UMAP_2, color = ab_level)) +
            geom_point(size = .1) +
            scale_color_viridis_c(labels = function(x) str_pad(x, 3),
...
mc_adt_plots <- plot_grid(plotlist = mc_prots_plot_list, ncol = 2)
mc_rna_plots <- plot_grid(plotlist = mc_genes_plot_list, ncol = 2)

#DE
de_df <- timep_gene_avg_expr %>%
  filter(gene_name %in% sle_candidates_label$gene_name) %>%
  mutate_at(vars(cluster, cluster_top),
            ~recode(., "IgG24" = "BCR", "RSQ24" = "TRL7"))

de_plot <- ggplot(de_df, aes(cluster, gene_name)) +
  geom_point(aes(size = pct, fill = scale_expr), 
             color = "black", shape = 21) +
  scale_size(range = c(0.25, 5), labels = scales::percent) +
  scale_fill_viridis_c(option = "magma") +
  scale_x_discrete(position = "top") +
  geom_text(data = distinct(de_df, gene_name, cluster = cluster_top),
            aes(label = gene_name), 
            hjust = 0, nudge_x = .1, size = 3) +
  theme_bw() +
  theme(axis.line = element_blank(),
        axis.ticks.y = element_blank(),
        axis.text.y = element_blank(),
  ...
  labs(x = NULL, y = NULL, 
       fill = "Scaled\nExpression", 
       size = "% of cells") +
   guides(fill = guide_colorbar(barwidth = .4, barheight = 3))


mc_out <- 
  plot_grid(mc_plot, testplot, mc_rna_plots, mc_adt_plots, 
            labels = c("A)", "B)", "C)", "D)"), nrow = 2) +
  theme(plot.background = element_rect(fill = "white", color = "white"),
        panel.border = element_blank())

ggsave("./microgrant.png", mc_out, width = 8, height = 5)
```
