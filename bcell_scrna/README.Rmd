...
qc1$layers[[1]] <- NULL
qc2$layers[[1]] <- NULL
qc1 <- qc1 + geom_point(size = .25)
qc2 <- qc1 + geom_point(size = .25)
...
make_umap_df <- function(x) {
	...
	select(barcode, clas = HTO_classification.global)
...
ggplot(umap_dat, aes(UMAP_1, UMAP_2)) +
    geom_point(aes(color = clas), size = .5) +
    scale_color_manual(values = c("Singlet" = "grey", "Doublet" = "black")) +
