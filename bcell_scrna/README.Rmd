...
hto_global_df <- tibble(barcode = colnames(hto_data),
       maxid = bcells@meta.data$HTO_maxID,
...
sle_coloc <- read_tsv("./summary_coloc_SLE-eQTLs.txt") %>%
  inner_join(features_df, by = c("molecular_trait_id" = "gene_id")) %>%
  select(gene_id = molecular_trait_id, gene_name)

#sle_candidates <- read_tsv("../de_gwas_candidates/sle_candidate_genes.tsv") %>%
#  mutate(gene_id= sub("\\.\\d+$", "", gene_id))

sle_candidates_24 <- timep_gene_avg_expr %>%
  inner_join(sle_coloc) %>%
...
```{r, echo=FALSE, fig.height=10, fig.width=8, eval=FALSE}
de_24_1 <- ggplot(timep_gene_avg_expr %>% filter(cluster_top == "IgG24"), 
  ...
  scale_fill_viridis_c(option = "magma") +
  scale_x_discrete(position = "top") +
  geom_text(data = sle_candidates_24 %>% filter(cluster == "IgG24"), 
            aes(label = i), 
...
de_24_2 <- ggplot(timep_gene_avg_expr %>% filter(cluster_top == "RSQ24"), 
       aes(cluster, gene_name)) +
  geom_point(aes(size = pct, fill = scale_expr), 
             color = "black", shape = 21) +
  scale_size(range = c(0.05, 2.5), labels = scales::percent) +
  scale_fill_viridis_c(option = "magma") +
  scale_x_discrete(position = "top") +
  geom_text(data = sle_candidates_24 %>% filter(cluster == "RSQ24"), 
            aes(label = i), 
            size = 3, nudge_x = -.1, nudge_y = -.25) +
  theme_bw() +
  theme(axis.line = element_blank(),
...
de_24_grid <- 
  plot_grid(de_24_2 + theme(legend.position = "none"),
            get_legend(de_24_2), 
            ncol = 1, rel_heights = c(.66, .34))

plot_grid(de_24_1 + theme(legend.position = "none"), 
          de_24_grid, nrow = 1)
```
...
```{r, echo=FALSE, fig.height=18, fig.width=8, eval=FALSE}
...
reported_genes <- read_lines("../de_gwas_candidates/reported_genes.tsv")
...
mc_genes <- c("BLK", "SOCS1", "NCF2", "CD44")
...
de_plot <- ggplot(de_24_df, aes(cluster, gene_name)) +
   geom_point(aes(size = pct, fill = scale_expr), 
             color = "black", shape = 21) +
...
mc_out <- 
  plot_grid(mc_plot, de_plot, mc_rna_plots, mc_adt_plots, 
