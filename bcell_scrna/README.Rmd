...
dn2_genes_quant <- bcells_singlet@assays$RNA@data %>% 
    ...
    pivot_longer(-c("gene_id", "gene_name"), 
                 names_to = "barcode", values_to = "gene_exp") %>%
    group_by(gene_id) %>%
    filter(mean(gene_exp > 0) > 0.025) %>%
    ungroup()

# shrink outliers
dn2_genes_quant_norm <- dn2_genes_quant %>%
  group_by(gene_id) %>%
  mutate(q01 = quantile(gene_exp, 0.005),
         q99 = quantile(gene_exp, 0.995),
         gene_exp = case_when(gene_exp < q01 ~ q01,
...
dn2_genes_plot_list <- umap_df %>%
    select(barcode, UMAP_1, UMAP_2) %>%
    left_join(dn2_genes_quant_norm, by = "barcode") %>%
    split(.$gene_name) %>%
    map(~ggplot(data = ., aes(UMAP_1, UMAP_2, color = gene_exp)) +
            geom_point(size = .2) +
            scale_color_viridis_c(labels = function(x) str_pad(x, 3),
...
```{r, echo=FALSE, fig.height=22, fig.width=10}
sle_genes <- 
...
tlr_genes_quant_norm <- tlr_genes_quant %>%
  group_by(gene_id) %>%
  mutate(q01 = quantile(gene_exp, 0.005),
         q99 = quantile(gene_exp, 0.995),
         gene_exp = case_when(gene_exp < q01 ~ q01,
...
umap_df %>%
    select(barcode, UMAP_1, UMAP_2) %>%
    left_join(tlr_genes_quant_norm, by = "barcode") %>%
    ggplot(aes(UMAP_1, UMAP_2, color = gene_exp)) +
    geom_point(size = .1) +
    scale_color_viridis_c(labels = function(x) str_pad(x, 3),
                          guide = guide_colorbar(barwidth = .5)) +
