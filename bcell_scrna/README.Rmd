...
magma_genes_annot <-
  read_tsv("./scDRS/data/gencode_genes/genes.v38.hg19.loc",
           col_names = FALSE) %>%
  select(gene_id = X1, gene_name = X6) %>%
  extract(gene_name, c("ix", "gene_name"), "(\\d+)_(.+)")

magma <- read_delim("./scDRS/results/Bentham.genes.out", delim = " ") %>%
  setNames(trimws(names(.))) %>%
  select(gene_id = GENE, z = ZSTAT) %>%
...
ggplot(magma_plot_df, aes(gene_name, abs_z)) +
  geom_point(size = 4, alpha = .5) +
  ggrepel::geom_text_repel(aes(label = gene_name), angle = 90, nudge_y = .5) +
  theme_minimal() +
  theme(axis.text.x = element_blank(),
        axis.ticks.x = element_blank(),
        panel.grid.minor = element_blank(),
...
scdrs_2 <- ggplot(umap_scdrs_df, aes(UMAP_1, UMAP_2, color = nlog10_pval)) +
    geom_point(size = .75) +
    scale_color_gradient(low = "lightyellow1", high = "red", 
