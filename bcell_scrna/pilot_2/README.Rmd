...
stim_class_df <- tibble(barcode = colnames(bcells_sng@assays$HTO@counts),
                        stim = bcells_sng@meta.data$HTO_maxID,
                        classif = bcells_sng@meta.data$HTO_classification.global)

hto_df <- bcells_sng@assays$HTO@counts %>%
    as.data.frame() %>%
    rownames_to_column("hto") %>%
    as_tibble() %>%
    pivot_longer(-hto, names_to = "barcode") %>%
...
plot_grid(admix_singlet, admix_doublet, ncol = 1)
```
...
go_top <- enrich_df %>%
    ...
    ungroup() %>%
    mutate(set = factor(set, levels = c("BCR", "TLR7", "BCR+TLR7", "DN2"))) %>%
    arrange(set, qvalue)
