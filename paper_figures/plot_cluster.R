...
plot_cluster <- function(
  ...
  labelTextSize=2.5 # orignally set as 5
  curveMax = 10
  curveExponent = 2
  yOffset = 0
  ...
  for( fancyVar in 1:length(groups) ){
    ...
    g <- ggplot2::ggplot() +
      ggplot2::geom_curve(data=allEdgesP, ggplot2::aes(x = start, xend = xtext, y = yOffset, yend = ytext, group = Group, colour = verdict, linewidth = curveMax * (log10counts - 1)^curveExponent ),
                 angle=90, curvature=-curv,lineend="round") +
      ggplot2::geom_curve(data=allEdgesP, ggplot2::aes(x = xtext, xend = end, y = ytext, yend = yOffset, group = Group, colour = verdict, linewidth = curveMax * (log10counts - 1)^curveExponent ),
                 angle=90, curvature=-curv,lineend="round") +
      ggplot2::geom_curve(data=allEdges, ggplot2::aes(x = start, xend = xtext, y = -yOffset, yend = ytext, group = Group, colour = verdict, linewidth = curveMax * (log10counts - 1)^curveExponent ),
                 angle=90,curvature=curv,lineend="round") +
      ggplot2::geom_curve(data=allEdges, ggplot2::aes(x = xtext, xend = end, y = ytext, yend = -yOffset, group = Group, colour = verdict, linewidth = curveMax * (log10counts - 1)^curveExponent ),
                 angle=90,curvature=curv,lineend="round") +
    ...
    new_theme_empty +
      ...
      ggplot2::geom_hline(yintercept=0, linewidth = centreLineWidth, colour = "white") +
      ggplot2::geom_hline(yintercept=0,alpha=.9, linewidth=1) +
      ...
      ggplot2::scale_linewidth(range=c(0.25,2.5), guide = 'none')
  ...
  plots[[1]] <- plots[[1]] +
    ggplot2::ggtitle(main_title[1]) +
    ggplot2::scale_colour_manual("", values = mainPalette ) +
    ggplot2::guides(colour=FALSE) +
    ggplot2::ylab(NULL) +
    ggplot2::theme(
      plot.margin = margin(0, 0, 0, 0),
      plot.title = element_text(hjust = .5, size = 9, margin = margin(0, 0, 0, 0)))
  plots[[2]] <- plots[[2]] +
    ggplot2::ggtitle(main_title[2]) +
    ggplot2::scale_colour_manual("", values = mainPalette ) +
    ggplot2::ylab(NULL) +
    ggplot2::theme(
      plot.margin = margin(0, 0, 0, 0),
  ...
  cowplot::plot_grid(plots[[2]], plots[[1]], ncol =1)
}