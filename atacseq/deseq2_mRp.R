...
pool_samples <- 
    ...
    filter(sample_id %in% samples_vec) |>
    pull(sample_id)
...
#dir.create("./results_deseq2/mRp")
save(dds, rld, file = "./results_deseq2/mRp/deseq2_data.Rdata")

## PCA
walk(c(500, 1000, 2500, 5000, 10000), 
     function(x) {
	 DESeq2::plotPCA(rld, intgroup = "condition", ntop = x, returnData = TRUE) |>
	 write_rds(glue("./results_deseq2/mRp/pcadata_{x}peaks.rds"))
     })
...
for (idx in 1:ncol(comparisons)) {
    ...
    write_tsv(results_out, glue("./results_deseq2/mRp/{treat_group}vs{control_group}.tsv"))
