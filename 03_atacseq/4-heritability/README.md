ATAC-seq: Partitioned Heritability (LDSC)
================

This directory contains the pipeline for integrating our ATAC-seq peak
data with genome-wide association study (GWAS) summary statistics using
**LD Score Regression (LDSC)**. This analysis determines whether genetic
variants associated with specific autoimmune traits are significantly
enriched within the open chromatin of our specific B cell stimulation
states.

**These scripts should be executed in the following order:**

#### Step 1. `setup.R` (Local R Script)

- **Action:** Prepares the data for LDSC. It uses UCSC `liftOver` to
  convert our master ATAC-seq peaks from GRCh38 to GRCh37 (hg19)
  coordinates. It extracts condition-specific significantly upregulated
  peaks, generates the configuration files required by LDSC-SEG, and
  parses our institutional database to compile a list of 37 immune,
  autoimmune, and control GWAS traits.
- **Input:** Differentially accessible peaks, MACS2 consensus peaks, a
  GRCh38-to-GRCh37 liftOver chain, and local LDSC metadata files.
- **Output:** GRCh37 peak coordinates, configuration files
  (`array_spec.tsv`, `peaksets.ldcts`), and a clean list of GWAS traits
  (`traits.txt`) saved to `./data/`.

#### Step 2. `make_annot.slurm` (Slurm Array Job)

- **Action:** Creates binary SNP annotations and computes LD scores.
  Iterating across 22 chromosomes for all custom peak sets, it maps 1000
  Genomes Phase 3 SNPs to our ATAC-seq peaks (using a 500bp window) and
  calculates the corresponding LD scores.
- **Input:** GRCh37 peak coordinates, 1000G Phase 3 BIM files, and
  HapMap3 SNP lists.
- **Output:** Chromosome-specific LD score files (`.l2.ldscore.gz`)
  stored in `./data/ldscores/`.

#### Step 3. `run_ldsc.slurm` (Slurm Array Job)

- **Action:** Executes the core heritability models. Iterating across
  the 37 formatted GWAS traits, it runs both Stratified LD Score
  Regression (**S-LDSC**, modeling joint enrichment of all states) and
  Cell-Type-Specific LD Score Regression (**LDSC-SEG**, modeling
  specific enrichment against a background control).
- **Input:** The pre-calculated LD scores from Step 2, baseline 1000G
  reference weights, and the target GWAS summary statistics.
- **Output:** Heritability coefficient files (`sldsc_*` and `ldscseg_*`)
  generated in the `./results/` directory.

#### Step 4. `format_results.R` (Local R Script)

- **Action:** Parses the raw log and text outputs from Step 3 to
  aggregate metrics across all 37 traits. It extracts annotation
  standard deviations and trait heritabilities to calculate the
  standardized effect size ($\tau^*$), applies FDR correction to the
  P-values, and compiles everything into a master summary table.
- **Input:** LDSC `.log`, `.results`, and `.cell_type_results.txt`
  files.
- **Output:** A combined and statistically adjusted enrichment table
...
### R Session Information
    ...
    ##  janitor       * 2.1.0      2021-01-05 [2] CRAN (R 4.1.1)
