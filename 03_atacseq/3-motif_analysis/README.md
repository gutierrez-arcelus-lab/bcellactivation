ATAC-seq: Transcription Factor Motif Analysis
================

This directory contains the pipeline for identifying transcription
factor binding motifs enriched within our differentially accessible
chromatin regions using **HOMER**.

**These scripts should be executed in the following order:**

#### Step 1. `setup.R` (R Script)

- **Action:** Parses the DESeq2 differential accessibility results to
  extract significantly upregulated peaks (`padj <= 0.01` and
  `log2FoldChange >= 1`) for each stimulation condition.
- **Input:** Pairwise differential accessibility tables
  (`*_24vsunst_24.tsv`) and the MACS2 consensus peak BED file.
- **Output:** Formatted target BED files (`./data/{stim}.bed`), the
  background BED file (`./data/consensus.bed`), and a `stims.txt` array
  index.

#### Step 2. `run_homer.slurm` (Slurm Array Job)

- **Action:** Executes HOMER (`findMotifsGenome.pl`) to identify
  vertebrate transcription factor motifs enriched at the differentially
  accessible peaks.
- **Input:** Target peak BED files, background consensus BED file, and
  the GRCh38 primary assembly genome FASTA.
- **Output:** Motif enrichment HTML reports and text summaries stored in
  condition-specific folders within `./results_vert/`.

#### Step 3. `process_results.R` (R Script)

- **Action:** Aggregates the individual HOMER `knownResults.txt` files
  across all stimulation conditions into a single master table.
- **Input:** The individual HOMER result text files.
- **Output:** A combined summary matrix (`results.tsv`) saved to the
  `./results_vert/` directory.
