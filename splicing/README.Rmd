...
In the plots above we see that HMGB1 has a strong AS effect, but no DTU. On the other hand, EIF4A2 has both AS and DTU.
...
load("./results/DN/DN.Rdata")



# HMGB1
hmgb1 <- as_tibble(introns) %>%
    filter(gene == "HMGB1") %>%
    filter(grepl("\\-$", clusterID)) %>%
    filter(clusterID == first(clusterID))
...
intron_cluster <- 
    tibble(event = letters[1:3],
           start = hmgb1$start,
           end = hmgb1$end) %>%
    mutate(transcript_id = "ENST00000399489")


ggplot(filter(plot_df, start > 30460000, end < 30463000)) +
    geom_segment(aes(x = start, xend = end, 
                     y = transcript_id, yend = transcript_id,
                     color = as.factor(col)), 
                 size = 5) +
    geom_curve(data = intron_cluster, 
...
```{r, fig.height=4}
#EIF4A2
eif4a2 <- as_tibble(introns) %>%
    filter(gene == "EIF4A2") %>%
    filter(grepl("\\+$", clusterID)) %>%
    filter(clusterID == first(clusterID))

intron_cluster_eif <- 
    tibble(event = letters[1:nrow(eif4a2)],
           start = eif4a2$start,
           end = eif4a2$end) %>%
    expand_grid(distinct(eif4a2, transcripts) %>% filter(!grepl("\\+", transcripts))) %>%
...
ggplot(filter(plot_df2, start > 186787500, end < 186790000)) +
    geom_segment(aes(x = start, xend = end, 
                     y = transcript_id, yend = transcript_id,
                     color = as.factor(col)), 
                 size = 5) +
    geom_curve(data = intron_cluster_eif, 
               aes(x = start, xend = end, 
                   y = transcript_id, yend = transcript_id), 
               curvature = -.1) +
    scale_color_manual(values = c("intron" = "grey80", "exon" = "blue"),
...
scharer_tpm <- 
    ...
    select(sample_id, status, transcript_id, gene_id, gene_name, tpm) %>%
    mutate(transcript_id = str_remove(transcript_id, "\\.\\d+$"))
    
scharer_tpm %>%
    filter(gene_name %in% c("HMGB1", "EIF4A2")) %>%
    ggplot(aes(x = tpm, y = reorder(transcript_id, tpm, "median"), fill = status)) +
...
sqtl_coloc <- read_tsv("./plot_data/coloc_sqtl.tsv") %>%
    ...
    select(dataset, intron, coloc_locus, gene)
