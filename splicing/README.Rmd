...
dtu_scharer <- 
    file.path("./results/scharer", cell_types, "dtu_perGene.tsv") %>%
    setNames(cell_types) %>%
    map_df(~read_tsv(.) %>%
...
```{r, fig.height=5}
top30_scharer <- leaf_scharer_df %>%
  filter(!is.na(genes)) %>%
  filter(cell_type == "DN") %>%
  top_n(30, abs(deltapsi)) %>%
  select(genes, deltapsi) %>%
  arrange(-abs(deltapsi)) %>%
  mutate(i = 1:n())

top30_barnas <- leaf_barnas_df %>%
  filter(!is.na(genes)) %>%
  top_n(30, abs(deltapsi)) %>%
  select(genes, deltapsi) %>%
  arrange(-abs(deltapsi)) %>%
...
top30_df <- bind_rows("Scharer" = top30_scharer, "Barnas" = top30_barnas, .id = "dataset")

ggplot(top30_df, aes(dataset, i)) +
  geom_point(aes(color = deltapsi), size = 3) +
  scale_color_gradient2(guide = guide_colorbar(barwidth = .5)) +
  geom_text(aes(label = genes), nudge_x = .05, hjust = 0, size = 3) +
  scale_y_reverse() +
  theme_bw() +
  theme(panel.grid = element_blank()) +
  labs(y = NULL, x = NULL)
