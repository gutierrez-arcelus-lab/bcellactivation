...
```{r, echo = FALSE, fig.height=14, fig.width=6}
labs_df <- leaf_df %>%
...
top_leaf_dtu <- leaf_dtu %>%
    filter(col == "TRUE") %>%
    mutate(p = pmin(adj_pvalue_leaf, adj_pvalue_dtu)) %>%
    group_by(cell_type) %>%
    top_n(25, -log10(p)) %>%
    mutate(shift_y = max(-log10(adj_pvalue_dtu)) - -log10(adj_pvalue_dtu)) %>%
    ungroup()

ggplot(leaf_dtu, aes(-log10(adj_pvalue_leaf), -log10(adj_pvalue_dtu))) +
    geom_point(aes(color = col), alpha = .5) +
