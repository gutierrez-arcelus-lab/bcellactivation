...
leaf_df <- read_tsv("./results/leafcutter_filtered.tsv")
...
labs_df <- leaf_df %>%
    ...
    group_by(cell_type) %>%
    filter(abs(deltapsi) >= quantile(abs(deltapsi), 0.95) |
           -log10(p.adjust) >= quantile(-log10(p.adjust), 0.95)) %>%
    ungroup() %>%
...
leaf_filtered <- read_tsv("./results/leafcutter_filtered_significant.tsv")
